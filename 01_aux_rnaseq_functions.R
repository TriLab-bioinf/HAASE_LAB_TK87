...
plot_normalized_gene_expression <- function(my_dds, ensmbl_gene_list, file_prefix){
  ...
  genes_of_interest_symbols <- replace_gene_acc_by_symbol_ids(ensmbl_gene_list, return_all = TRUE, db = org.Hs.eg.db)
...
print_gene_expression <- function(dds, gene_id, symbol = "", title_prefix = ""){
  counts.p <- plotCounts(dds, gene = gene_id, intgroup = c("Sample"), 
  ...
  p <- ggerrorplot(counts.p, x = "Sample", y = "count",
                   desc_stat = "mean_sd", 
                   color = "Sample",
                   add = "dotplot", 
...
print_gene_expression_over_timme <- function(ddsTC, gene_id, symbol = "", title_prefix = ""){
  ...
  fiss <- plotCounts(ddsTC, gene_id, intgroup = c("Sample"), returnData = TRUE)
  fiss$minute <- as.numeric(as.character(fiss$Time))
  p <- ggplot(fiss,
         aes(x = Time, y = count, color = Sample, group = Sample)) + 
